...
void RLDFilter(cv::Mat &imgb, cv::Mat &kernel, cv::Mat &imgx) {
  ...
  cv::Mat img3 = imgb.clone();
  cv::Mat img4 = imgb.clone();
  cv::Mat img5 = imgb.clone();
  ...
  do {
    // Mk=H*Ik
    BlurrPBCsv(img2, kernel, img4, 0);
    // TODO: Fixes division by zero
    img4 += 1e-10;

    cv::subtract(img4, imgb, img5);

    // D/Mk
    cv::divide(imgb, img4, img4);
    // Ht*(D/Mk)
    ...
    cv::absdiff(img5, cv::Scalar(1.0), img5);
    norm = cv::norm(img5, cv::NORM_L2) / ((img5.cols) * (img5.rows));
    if (it > 1) {
      delt = norm - oldnorm;
      oldnorm = norm;
    } else {
    ...
    cv::imshow("Processing...", img3);
    char c = cv::waitKey(10);
    if (c == 27) break;

    it++;
    cout << it << "  " << delt << "  " << norm << '\n';
  } while ((abs(delt) > 1e-8) && (it < 101));
...
void Process(int pos, void *) {
  ...
  if (imgi.channels() != 1) {
    ...
    if (flag) {
      ...
      RLDFilter(imgc1b, kernel, imgc1d);
      RLDFilter(imgc2b, kernel, imgc2d);
      RLDFilter(imgc3b, kernel, imgc3d);
    }
    // merge and show

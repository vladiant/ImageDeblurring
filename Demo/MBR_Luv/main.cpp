...
int main(int argc, char *argv[]) {
  ...
  // convert to CIE Luv
  if (imgi.channels() != 1) cv::cvtColor(imgi, imgi, cv::COLOR_BGR2Luv);
  ...
  if (imgi.channels() != 1) {
    ...
    // CIE Luv specific treatment for 8 bit image; 16 bit not yet implemented
    imgc1i.convertTo(imgc1, CV_32F);
    imgc1 = imgc1 * 100.0 / sca;
    imgc2i.convertTo(imgc2, CV_32F);
    imgc2 = imgc2 * 354.0 / sca - 134;
    imgc3i.convertTo(imgc3, CV_32F);
    imgc3 = imgc3 * 256.0 / sca - 140;
    // creation of channel images for blurring
...
void Process(int pos, void *) {
  ...
  if (imgi.channels() != 1) {
    ...
    // convert to RGB
    cv::cvtColor(img3b, img3b, cv::COLOR_Luv2BGR);
    ...
    do {
      ...
      */
      ...
      imgcd(cv::Rect(kernel.cols, kernel.rows, imgi.cols, imgi.rows))
          .copyTo(img3d);
      // convert to RGB
      cv::cvtColor(img3d, img3d, cv::COLOR_Luv2BGR);

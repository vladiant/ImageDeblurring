...
float stdev1, stdev2,
    stdev3;  // estimated by Fourier analysis standard noise deviation

const float NOIS_SC = 1.0;  // scale for the noise filtering - to be estimated!
...
void Process(int pos, void *) {
  ...
  if (imgi.channels() != 1) {
    ...
    // deblurring and noise filtering - each channel separately
    stdev1 = NoiseDev(imgc1b);
    FDFilter(imgc1b, imgb, imgc1d,
             NOIS_SC * sqrt(stdev1) / log(imgc1b.cols * imgc1b.rows));
    stdev2 = NoiseDev(imgc2b);
    FDFilter(imgc2b, imgb, imgc2d,
             NOIS_SC * sqrt(stdev2) / log(imgc2b.cols * imgc2b.rows));
    stdev3 = NoiseDev(imgc3b);
    FDFilter(imgc3b, imgb, imgc3d,
             NOIS_SC * sqrt(stdev1) / log(imgc3b.cols * imgc3b.rows));

...
int main(int argc, char *argv[]) {
  ...
  // convert to CIE Lab
  if (imgi.channels() != 1) cv::cvtColor(imgi, imgi, cv::COLOR_BGR2Lab);
  ...
  if (imgi.channels() != 1) {
    ...
    // CIE Lab specific treatemnt for 8 bit images; 16 bit not implemented
...
void Process(int pos, void *) {
  ...
  if (imgi.channels() != 1) {
    ...
    // convert to RGB
    cv::cvtColor(img3b, img3b, cv::COLOR_Lab2BGR);
    ...
    do {
      ...
      */
      ...
      imgcd(cv::Rect(kernel.cols, kernel.rows, imgi.cols, imgi.rows))
          .copyTo(img3d);
      // convert to RGB
      cv::cvtColor(img3d, img3d, cv::COLOR_Lab2BGR);

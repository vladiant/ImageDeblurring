...
// Version     :
// Description : Program to test LSQR motion deblurring
//             : Algorithm based on paper:
//             : LSQR: An algorithm for sparse linear equations and sparse
//             : least squares, C. C. Paige and M. A. Saunders,  TOMS 8(1),
//             : 43-71 (1982).
...
int main(int argc, char *argv[]) {
  ...
  /// imgx norm calculation
  float zeta = 0, sn2 = 0, cs2 = -1, xxnorm;
  ...
  do {
    ...
    // imgx norm calculation
    float delta = sn2 * rho;
    float gammabar = -cs2 * rho;

    float s = betha / rho;
    float Theta = s * alpha;

    // imgx norm calculation
    float gamma = sqrt(gammabar * gammabar + Theta * Theta);
    cs2 = gammabar / gamma;
    sn2 = Theta / gamma;
    ...
    xxnorm += 2 * zeta * zeta;
    cout << xxnorm << "  " << cv::norm(imgx, cv::NORM_L2) << endl;

...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      ((float *)(imgf.data + row * imgf.step))[col] = 1.0 - (a1 / smax);
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  for (int row = 0; row < imgb.rows; row++) {
    for (int col = 0; col < imgb.cols; col++) {
      float a1 = ((float *)(imgls.data + row * imgls.step))[col];
      ((float *)(imglr.data + row * imglr.step))[col] = 1.0;  //-a1;
    }
  }
    ...
    for(int row=0;row<imgb.rows;row++)
      {
            for(int col=0;col<imgb.cols;col++)
        {
                    float a1=((float*)(imgls.data +
    row*imgls.step))[col];
                    ((float*)(imglr.data +
    row*imglr.step))[col]=1.0-a1;
            }
      }
    */

 ...
 * Author:	Lagendijk, R.L., Promotor: 	Biemond, J.
 * Date:	1990-04-26
...
void Rset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  int i, j, its;
  const int x1 = 2, y1 = 2;  // size of the scan area
  float s1, s2, p1, smax = 0;
  const float mu = 0.0001;  // tunable constant
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      s1 = 0;
      s2 = 0;

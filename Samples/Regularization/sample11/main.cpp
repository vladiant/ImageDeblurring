...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  ...
  const float mu = 0.5, etha = 6.0;  // tunable constant
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      row1 = y1;
      for (col1 = -x1 + 1; col1 < x1; col1++, its++) {
        // replicate boundary conditions
        if ((row + row1) < 0)
          i = 0;
        else if ((row + row1) > (imga.rows - 1))
      ...
      s1 /= its;
      Tij = etha * 0.75 * so2 / its;

      row1 = -y1;
      for (col1 = -x1 + 1; col1 < x1; col1++) {
        // replicate boundary conditions
        if ((row + row1) < 0)
          i = 0;
        else if ((row + row1) > (imga.rows - 1))
      ...
      if (s2 < Tij) {
        x1 = (Nmax < (x1 + 1)) ? Nmax : x1 + 1;
        y1 = (Nmax < (y1 + 1)) ? Nmax : y1 + 1;
      } else {
        x1 = (Nmin > (x1 - 1)) ? Nmin : x1 - 1;
        y1 = (Nmin > (y1 - 1)) ? Nmin : y1 - 1;
      }
    }
  }

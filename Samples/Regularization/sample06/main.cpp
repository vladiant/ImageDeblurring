 ...
 * Proceedings, IEEE International Conference on Image Processing(ICIP'94),
 * Nov 13-16, 1994, Austin, Texas, U.S.A., Vol.II, pp.670-674.
 */
...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      a1 = (a1 > so2) ? a1 : so2;
      ((float *)(imgf.data + row * imgf.step))[col] = 1.0 / (0.5 * a1);
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  Sset(imgl, imglr, stdev * stdev);
  for (int row = 0; row < imglr.rows; row++) {
    for (int col = 0; col < imglr.cols; col++) {
      float a1 = ((float *)(imglr.data + row * imglr.step))[col];
      ((float *)(imglr.data + row * imglr.step))[col] = a1 / (1.0 + a1);
    }
  }
  ...
  // alpha calculated as proposed in this paper, first part
  alpha = 0;
  ...
  // alpha calculated as proposed in this paper, second part
  gam = 0;
  for (int row = 0; row < imgb.rows; row++) {
    for (int col = 0; col < imgb.cols; col++) {
  ...
  do {
    ...
    // alpha calculated as proposed in this paper, second part
    gam = 0;
    for (int row = 0; row < imgl.rows; row++) {
      for (int col = 0; col < imgl.cols; col++) {
        float a1 = ((float *)(imgl.data + row * imgl.step))[col];
        gam += a1 * a1;
      }
    }
    alpha = alpha * 5e-6 / gam;

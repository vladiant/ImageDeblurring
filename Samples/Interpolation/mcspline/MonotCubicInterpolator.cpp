...
MonotCubicInterpolator::MonotCubicInterpolator(const vector<double>& x,
...
bool MonotCubicInterpolator::read(const std::string& datafilename, int xColumn,
                                  int fColumn) {
  data.clear();
  ddata.clear();
...
void MonotCubicInterpolator::addPair(double newx, double newf) {
  if (std::isnan(newx) || std::isinf(newx) || std::isnan(newf) ||
      std::isinf(newf)) {
...
double MonotCubicInterpolator::evaluate(double x) const {
  ...
  if (ddata.size() != data.size()) {
    double finterp = xf1.second + (xf2.second - xf1.second) /
                                      (xf2.first - xf1.first) * (x - xf1.first);
    return finterp;
  } else {  // Do Cubic Hermite spline
    double t = (x - xf1.first) / (xf2.first - xf1.first);  // t \in [0,1]
    double h = xf2.first - xf1.first;
    double finterp = xf1.second * H00(t) + ddata[xf1.first] * H10(t) * h +
                     xf2.second * H01(t) + ddata[xf2.first] * H11(t) * h;
    return finterp;
  }
}
...
string MonotCubicInterpolator::toString() const {
  ...
  for (map<double, double>::const_iterator it = data.begin(); it != data.end();
       ++it) {
...
pair<double, double> MonotCubicInterpolator::getMissingX() const {
  if (data.size() < 2) {
  ...
  for (xf_iterator = data.begin(), xf_next_iterator = ++(data.begin());
       xf_next_iterator != data.end(); ++xf_iterator, ++xf_next_iterator) {
  ...
  double newXvalue =
      ((*maxfDiffPair_iterator).first + ((*(++maxfDiffPair_iterator)).first)) /
      2;
  return make_pair(newXvalue, maxfDiffValue);
}

pair<double, double> MonotCubicInterpolator::getMaximumF() const {
  if (data.size() <= 1) {
...
pair<double, double> MonotCubicInterpolator::getMinimumF() const {
  if (data.size() <= 1) {
...
void MonotCubicInterpolator::computeInternalFunctionData() const {
  ...
  strictlyMonotone =
      true;  // We assume this is true, and will set to false if not
  monotone = true;
  ...
  if (xf_next_iterator != data.end()) {
    if (xf_iterator->second > xf_next_iterator->second) {
      ...
      strictlyDecreasing = true;  // if strictlyMonotone == false, this one
                                  // should not be trusted anyway
      decreasing = true;
      strictlyIncreasing = false;
      increasing = false;
...
//            (2,3), (3,4), (4,5)
//
//       Assumes at least 3 datapoints. If less than three, this function is a
//       noop.
...
//           (1,2), (2,3), (3,4), (5,5), (6,6)
//
//       Assumes at least two datapoints, if one or zero datapoint, this is a
//       noop.
//
//
void MonotCubicInterpolator::shrinkFlatAreas(const double epsilon) {
  ...
  while (xf_next_iterator != data.end()) {
    // cout << xf_iterator->first << "," << xf_iterator->second << " " <<
    // xf_next_iterator->first << "," << xf_next_iterator->second << "\n";
    if (fabs(xf_iterator->second - xf_next_iterator->second) < epsilon) {
      // cout << "erasing data pair" << xf_next_iterator->first << " " <<
      // xf_next_iterator->second << "\n";
...
void MonotCubicInterpolator::computeSimpleDerivatives() const {
  ...
  xf_iterator = data.begin();
  xf_next_iterator = ++(data.begin());
  diff = (xf_next_iterator->second - xf_iterator->second) /
  ...
  if (data.size() > 2) {
    ...
    for (intpoint = ++data.begin(); intpoint != --data.end(); ++intpoint) {
      ...
      */
      ...
      map<double, double>::const_iterator lastpoint = intpoint;
      --lastpoint;
      map<double, double>::const_iterator nextpoint = intpoint;
      ++nextpoint;
...
void MonotCubicInterpolator::adjustDerivativesForMonotoneness() const {
  ...
  for (point = data.begin(), dpoint = ddata.begin(); point != --data.end();
       ++point, ++dpoint) {
    ...
    double delta =
        (nextpoint->second - point->second) / (nextpoint->first - point->first);
    if (fabs(delta) < 1e-14) {
...
void MonotCubicInterpolator::scaleData(double factor) {
  ...
  if (data.size() == ddata.size()) {
    for (it = data.begin(), itd = ddata.begin(); it != data.end();
         ++it, ++itd) {
      it->second *= factor;
      itd->second *= factor;
    };

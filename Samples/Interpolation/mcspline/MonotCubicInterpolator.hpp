...
/**
   ...
   that can be interpolated using monotone cubic interpolation
...
class MonotCubicInterpolator {
 public:
  /**
     @param datafilename A datafile with the x values and the corresponding f(x)
     values
     ...
     Ignores all lines not conforming to
     \<whitespace\>\<float\>\<whitespace\>\<float\>\<whatever\>\<newline\>
  */
  ...
  */
  ...
  /**
      ...
      that file.
  */
  MonotCubicInterpolator(const std::string& datafilename, int xColumn,
                         int fColumn) {
  ...
  */
  ...
  /**
     ...
     @return New x value beleived to yield the best improvement in global
     accuracy
  ...
  */
  void chopFlatEndpoints() { chopFlatEndpoints(1e-14); }
  ...
  */
  ...
  /**
     ...
     providing a default epsilon parameter
  */
  void shrinkFlatAreas() { shrinkFlatAreas(1e-14); };
 ...
 private:
  ...
  // if strictlyMonotone is true (and can be trusted), the two next are
  // meaningful
  ...
  */
  ...
  /**
      Computes initial derivative values using centered (second order)
     difference for internal datapoints, and one-sided derivative for endpoints

      The internal datastructure map<double,double> ddata is populated by this
     method.
  */

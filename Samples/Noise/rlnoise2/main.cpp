...
int main(int argc, char *argv[]) {
  ...
  cv::Mat imgr1;                // regularized images: |A.x-b|
  ...
  do {
    do {
      ...
      if ((delt < 0)) {
        ...
        step *= 1.1;
    } while (((delt < 0)) && (step > 1e-5));

    oldnorm = norm;

    if (delt > 0) img2 = img3.clone();
  ...
  imgr1 = img3.clone();
  cv::namedWindow("|Ax-b|", cv::WINDOW_AUTOSIZE);
  cv::imshow("|Ax-b|", imgr1);

...
Kernel kernel_type = Kernel::Erfc;

void hLineKernel(cv::Mat &imgb, int fl = 0) {
  imgb = 0;
  float s4 = 0;
  float s5 = 0;
  ...
  if (fl == 0) {
    for (int col = 0; col < imgb.cols / 2; col++) {
      ...
      s4 += s5;
      // cout << s5 << '\n';
      ((float *)(imgb.data + imgb.rows / 2 * imgb.step))[col + imgb.cols / 2] =
          s5;
    }
    ...
    if (fl == 0) {
      switch (kernel_type) {
        case Kernel::MoveXY:
          ksetMoveXY(imgb, 3, 0);
          break;
        case Kernel::MoveXYxw:
          ksetMoveXYxw(imgb, 3, 0);
          break;
        case Kernel::MoveXYb:
          ksetMoveXYb(imgb, 3, 0);
          break;
        case Kernel::MoveXYbcont:
          ksetMoveXYbcont(imgb, 3, 0);
          break;
        case Kernel::Defocus:
          ksetDefocus(imgb, 3);
          break;
...
int main(int argc, char *argv[]) {
  ...
  const int x1 = 10, y1 = 0;  // Define vector of motion blurr
  ...
  kernel = cv::Mat(2 * ksize + 1, 2 * ksize + 1, CV_32FC1);
  ...
  */
  ...
  if (argc >= 3) {
    switch (argv[2][0]) {
      case 'm':
        std::cout << "Move kernel\n";
        kernel_type = Kernel::MoveXY;
        break;
      case 'w':
        std::cout << "Move kernel\n";
        kernel_type = Kernel::MoveXYxw;
        break;
      case 'b':
        std::cout << "Move kernel\n";
        kernel_type = Kernel::MoveXYb;
        break;
      case 'c':
        std::cout << "Move kernel\n";
        kernel_type = Kernel::MoveXYbcont;
        break;
      case 'd':
        std::cout << "Defocus kernel\n";
        kernel_type = Kernel::Defocus;
        break;
      case 'l':
        std::cout << "Simple line kernel\n";
        kernel_type = Kernel::Classic;
        break;
      case 'g':
        ...
        break;
      case 'e':
        std::cout << "Erfc kernel\n";
        kernel_type = Kernel::Erfc;
        break;
      default:
        std::cout << "No kernel\n";
    }

 ...
 *
 * Kernel calculated at predefined steps
 *
 * Regularization added for noise supression:
 * alpha calculated as defined in:
 * Spatially Adaptive Iterative Algorithm for the Restoration
 * of Astronomical Images
 * Aggelos K. Katsaggelos, Moon Gi Kang
 * International Journal of Imaging Systems and Technology
 * 6 (1995) 305-313
 *
 * regularization weights defined in:
 * Two Iterative Image Restoration Algorithms with Applications to Nuclear
 * Medicine Charalambous C, Ghaddar FK, Kouris K. IEEE Trans. Med. Imaging. 11
 * (1992) 2-8
 *
...
const int kernel_stepx = 5;
const int kernel_stepy = 5;
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      if (((row % kernel_stepy) == 0) && ((col % kernel_stepx) == 0)) {
        ...
        // sparse kernel
        dims[0] = 2 * int(abs(ybmax) + 0.5) + 11;
        dims[1] = 2 * int(abs(xbmax) + 0.5) + 11;
        skernel = cv::SparseMat(2, dims, CV_32F);

        // set the kernel values
        for (int jk = 0; jk < bsamples - 1; jk++) {
          // float step=1.0/bstep[jk];
          for (int jj = 0; jj < bstep[jk]; jj++) {
            float xd = -((yb[jk + 1] - yb[jk]) * jj / bstep[jk] + yb[jk]) +
                       dims[0] / 2;
            float yd = -((xb[jk + 1] - xb[jk]) * jj / bstep[jk] + xb[jk]) +
                       dims[1] / 2;
...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  int i, j, its;
  const int x1 = 2, y1 = 2;  // size of the scan area
  float s1, s2, p1, smin = 1000;
  const float u = 0.4;  // tunable constant
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      s1 = 0;
      s2 = 0;
  ...
  if (smin < so2) smin = so2;

  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      float a1 = ((float *)(imgf.data + row * imgf.step))[col];
      ((float *)(imgf.data + row * imgf.step))[col] = pow((smin / a1), u);
    }
  }
}

/*
 * R weight matrix set up
 */

void Rset(cv::Mat &imga, cv::Mat &imgf) {
  int i, j, its;
  const int x1 = 2, y1 = 2;  // size of the scan area
  float s1, s2, p1, smax = 0;
  const float v = 0.1;  // tunable constant
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      s1 = 0;
      s2 = 0;
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      float a1 = ((float *)(imgf.data + row * imgf.step))[col];
      ((float *)(imgf.data + row * imgf.step))[col] = pow((a1 / smax), v);
    }
  }
}
...
void CGDFilter(cv::Mat &imgb, int bsamples, float *timeb, float *Thetabx,
               float *Thetaby, float *Thetabz, float time_capture_start,
               cv::Mat &imgx) {
  int it;  // iteration counter
  double gk, bk, norm,
      oldnorm;  // Temp for dot product, regularization coefficient
  float stdev, alpha, gam,
      s1;  // Standard noise deviation, regularization constant, scalar product
           // of regularized blurred image, temporal sum

  cv::namedWindow("Processing...", 0);

  stdev = NoiseDev(imgb) * 1500 / 3.12;
  alpha = 1e-6;  // initial value

  // initial approximation of the restored image
  imgb.copyTo(imgx);

  // regularization parameters
  cv::Mat imgl = imgb.clone();

  // regularization weights
  cv::Mat imgls = imgb.clone();
  cv::Mat imglr = imgb.clone();
  Sset(imgb, imgls, stdev * stdev);
  Rset(imgb, imglr);

  // initial approximation of the residual
  cv::Mat imgr = imgb.clone();
  BlurrPBCsv(imgx, bsamples, timeb, Thetabx, Thetaby, Thetabz,
             time_capture_start, imgr);
  cv::subtract(imgb, imgr, imgr);
  BlurrPBCsv(imgr, bsamples, timeb, Thetabx, Thetaby, Thetabz,
             time_capture_start, imgr, 1);

  // add regularization
  cv::Laplacian(imgb, imgl, -1);

  // paper based estimation of alpha, initial part
  gam = 0;
  ...
  do {
    ...
    // paper based estimation of alpha, part one
    alpha = 0;
    for (int row = 0; row < imgb.rows; row++) {
      for (int col = 0; col < imgb.cols; col++) {
    ...
    // paper based estimation of alpha, part two
    s1 = 0;
    for (int row = 0; row < imgb.rows; row++) {
      for (int col = 0; col < imgb.cols; col++) {
        float a1 = ((float *)(imgl.data + row * imgl.step))[col];
    ...
    // beta_k
    BlurrPBCsv(imgp, bsamples, timeb, Thetabx, Thetaby, Thetabz,
               time_capture_start, imgap);
    // regularization
    ...
    // weight matrices S and R updated
    ...
    it++;
    cout << it << "  " << norm << '\n';
  } while ((norm > 1e-8));  //((norm>1e-8)&&((norm<oldnorm)||(it<10)));
...
void Process(int pos, int bsamples, float *timeb, float *Thetabx,
  ...
  if (imgi.channels() != 1) {
    ...
    CGDFilter(imgc1, bsamples, timeb, Thetabx, Thetaby, Thetabz,
              time_capture_start, imgc1d);
    CGDFilter(imgc2, bsamples, timeb, Thetabx, Thetaby, Thetabz,
              time_capture_start, imgc2d);
    CGDFilter(imgc3, bsamples, timeb, Thetabx, Thetaby, Thetabz,
...
// ./regularization_12 Lenna.png < IMG_20000109_195543.jpg.txt

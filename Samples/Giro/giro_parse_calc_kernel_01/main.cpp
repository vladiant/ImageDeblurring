...
const float FOV = 2592 * 1.0 * 195.5 / 189;  // 0.9 for test cases ?!

// time delay for the timestamps - to be calibrated
const float time_delay = 0.017713;
...
int main(int argc, char *argv[]) {
  ...
  // image processing
  cv::Mat imgi =
      cv::imread(argv[1], cv::IMREAD_ANYDEPTH | cv::IMREAD_ANYCOLOR);
  ...
  int r0x = 2031, r0y = 1641;  // pixel where the movement points will be drawn
  ...
  for (int j=0;j<N; j++)
  {
          ...
          if (j>0)
          {
                  ...
                  //calculation of the rotation matrix, based on actual angle,
                  //to be applied to initial point
                  float urx=Thetax[j], ury=Thetay[j], urz=Thetaz[j];
                  float sur=sqrt(urx*urx+ury*ury+urz*urz);
                  urx/=sur; ury/=sur; urz/=sur;
                  ...
                  if
  (!((time[j]>time_capture_start)&&(time[j]<time_capture_end))) cout << "# ";

                  x1=(Rt[0]*x0+Rt[1]*y0+Rt[2]*1.0)/(Rt[6]*x0+Rt[7]*y0+Rt[8]*1.0);
                  y1=(Rt[3]*x0+Rt[4]*y0+Rt[5]*1.0)/(Rt[6]*x0+Rt[7]*y0+Rt[8]*1.0);
                  cout << time[j] << "  " << x1*(FOV)+u0-r0x << "  " <<
  y1*(FOV)+v0-r0y << '\n';
          }
          else
          {
                  if
  (!((time[0]>time_capture_start)&&(time[0]<time_capture_end))) cout << "# ";
                  x1=(1.0*x0+0.0*y0+0.0*1.0)/(x0*0.0+y0*0.0+1.0*1.0);
                  y1=(0.0*x0+1.0*y0+0.0*1.0)/(0.0*x0+0.0*y0+1.0*1.0);
                  cout << time[0] << "  " << x1*(FOV)+u0-r0x << "  " <<
  y1*(FOV)+v0-r0y << '\n';
          }
  }
  */
  ...
  /*
  for (int j=0;j<bsamples-1; j++) cout << bstep[j] << "   ";
  cout <<'\n' << bsteps << '\n';
  */

  // cout << int(xbmax+0.5) << "  " << int(ybmax+0.5) << '\n';

  // draw the kernel for this point
  cv::Mat kernel = cv::Mat(
      cv::Size(2 * abs(int(ybmax + 0.5)) + 11, 2 * abs(int(xbmax + 0.5)) + 11),
      CV_32FC1);
  kernel = 0;
  ...
  for (int j = 0; j < bsamples - 1; j++) {
    ...
    for (int jj = 0; jj < bstep[j]; jj++) {
      ...
      ((float *)(kernel.data +
                 int(yd + 1) * (kernel.step)))[int(xd)] +=
          (int(xd) - xd + 1) * (yd - int(yd)) * step;
      ((float *)(kernel.data +
                 int(yd) * (kernel.step)))[int(xd + 1)] +=
          (xd - int(xd)) * (int(yd) - yd + 1) * step;
      ((float *)(kernel.data +
                 int(yd + 1) * (kernel.step)))[int(xd + 1)] +=
  ...
  // save the kernel for the center
  kernelsum = 0;
  for (int row = 0; row < kernel.rows; row++)
    for (int col = 0; col < kernel.cols; col++)
      if (kernelsum <

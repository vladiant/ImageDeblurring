...
class GyroBlurKernel {
 ...
 public:
  ...
  // Gets the blur parameters and puts them in structure
  GyroDeblurParams GetBlurParameters(void);

  // Resets the blur parameters
  void SetBlurParameters(GyroDeblurParams Params);
};
...
void GyroBlurKernel::KernelInterpolation(int Row, int Col) {
  ...
  timeb.resize(bsamples);
  xb.resize(bsamples);
  yb.resize(bsamples);
  bstep.resize(bsamples - 1);
  isInterpolated = true;
...
void GyroBlurKernel::MaxBlockSize(int& xkspan, int& ykspan) {
  ...
  do {
    ...
    for (int row = 0; row < ImageHeight - stepy; row += stepy)
      for (int col = 0; col < ImageHeight - stepx; col += stepx) {
        KernelInterpolation(row, col);
        float xk1 = xbmax;
        float yk1 = ybmax;
        ...
        diffy = std::max(diff1, diff2);
        diffy = std::max(diffy, diff3);
      }
    ...
    } while (diff < diffy);  // smooth change
...
// Gets the blur parameters and puts them in structure
GyroDeblurParams GyroBlurKernel::GetBlurParameters(void) {
  GyroDeblurParams value;

  value.TimeCapture = TimeCapture;
  value.TimeExposure = TimeExposure;
  value.TimeReadout = TimeReadout;
  value.TimeDelay = TimeDelay;
  value.FOVx = FOVx;
  value.FOVy = FOVy;
  value.OptCentX = OptCentX;
  value.OptCentY = OptCentY;
...
// Resets the blur parameters
void GyroBlurKernel::SetBlurParameters(GyroDeblurParams Params) {
  // Initial initialization
  TimeCapture = Params.TimeCapture;
  TimeExposure = Params.TimeExposure;
  TimeReadout = Params.TimeReadout;

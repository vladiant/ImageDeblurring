...
//             : using a gyro data calculated kernel with CG approximation
//             : Boundaries condition: replicate
//             : Gyro data read from cin (can be given from file)
//             : Rolling shutter correction added
//             : Regularization added for noise supression: empirical GCV
...
void CGDFilter(cv::Mat &imgb, cv::Mat &imgx, int Ns,
               DiscreteAngularVelocitiesIntegrator<Versor<float>, float>
                   &angularVelocityIntegrator,
               Time capture_timestamp, Time time_exposure, Time time_readout,
               Time time_delay) {
  ...
  BlurrPBCsv(imgx, imgr, Ns, angularVelocityIntegrator, capture_timestamp,
             time_exposure, time_readout, time_delay);
  cv::subtract(imgb, imgr, imgr);
  BlurrPBCsv(imgr, imgr, Ns, angularVelocityIntegrator, capture_timestamp,
  ...
  do {
    ...
    // beta_k
    BlurrPBCsv(imgp, imgap, Ns, angularVelocityIntegrator, capture_timestamp,
               time_exposure, time_readout, time_delay);
    // regularization
...
void Process(int pos, int Ns,
  ...
  if (imgi.channels() != 1) {
    ...
    // BlurrPBCsv(imgc1, imgc1d, Ns, angularVelocityIntegrator,
    // capture_timestamp, time_exposure, time_readout, time_delay);
    // BlurrPBCsv(imgc2, imgc2d, Ns, angularVelocityIntegrator,
    // capture_timestamp, time_exposure, time_readout, time_delay);
    // BlurrPBCsv(imgc3, imgc3d, Ns, angularVelocityIntegrator,
    // capture_timestamp, time_exposure, time_readout, time_delay);
    CGDFilter(imgc1, imgc1d, Ns, angularVelocityIntegrator, capture_timestamp,
              time_exposure, time_readout, time_delay);
    CGDFilter(imgc2, imgc2d, Ns, angularVelocityIntegrator, capture_timestamp,
              time_exposure, time_readout, time_delay);
    CGDFilter(imgc3, imgc3d, Ns, angularVelocityIntegrator, capture_timestamp,
              time_exposure, time_readout, time_delay);

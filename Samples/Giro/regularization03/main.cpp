 ...
 * Steven S. O. Choy, Yuk-Hee Chan and Wan-Chi Siu
 *
...
/*
 * S matrix (regularization weights) set up
 */
...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      ((float *)(imgf.data + row * imgf.step))[col] = 1.0 / (1.0 + (a1 / so2));
    }
  }
}
...
void CGDFilter(cv::Mat &imgb, int bsamples, float *timeb, float *Thetabx,
  ...
  do {
    ...
    it++;
    cout << it << "  " << norm << '\n';
  } while ((norm > 1e-7));  //((norm>1e-8)&&((norm<oldnorm)||(it<10)));

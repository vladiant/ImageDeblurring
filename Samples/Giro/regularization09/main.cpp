...
// Version     :
// Description : Demo program used to demonstrate the motion blur for images
// using a gyro data calculated kernel with Lucy-Richardson algorithm
...
//
// Regularization added for noise supression: alpha - empirical GCV
//
//============================================================================
...
void BlurReBC(cv::Mat &imga, int bsamples, float *timeb, float *Thetabx,
...
void RLDFilter(cv::Mat &imgb, int bsamples, float *timeb, float *Thetabx,
               float *Thetaby, float *Thetabz, float time_capture_start,
               cv::Mat &imgx) {
  ...
  do {
    ...
    BlurReBC(imgx, bsamples, timeb, Thetabx, Thetaby, Thetabz,
             time_capture_start, img1);
    // g/D.x - dividing blurred image on blurred x
    cv::divide(imgb, img1, img2);
    // Dt.(g/D.x) - blur again with transposed operator
    BlurReBC(img2, bsamples, timeb, Thetabx, Thetaby, Thetabz,
             time_capture_start, img1, 1);
...
void Process(int pos, int bsamples, float *timeb, float *Thetabx,
  ...
  if (imgi.channels() != 1) {
    ...
    RLDFilter(imgc1, bsamples, timeb, Thetabx, Thetaby, Thetabz,
              time_capture_start, imgc1d);
    RLDFilter(imgc2, bsamples, timeb, Thetabx, Thetaby, Thetabz,
              time_capture_start, imgc2d);
    RLDFilter(imgc3, bsamples, timeb, Thetabx, Thetaby, Thetabz,

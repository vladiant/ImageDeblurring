...
int fft(float *x_in, float *x_out, int n, int shift) {
  ...
  alpha = (float)(2.0 * M_PI / ((float)n));
  beta = (float)(2.0 * M_PI / ((float)n1));
  gamma = (float)(2.0 * M_PI / ((float)n2));
  for (k1 = 0; k1 < n1; k1++) {
    tmpBeta = beta * k1;
    for (k2 = 0; k2 < n2; k2++) {
      idx = shift * (n2 * k1 + k2);
...
int fftInverse(float *x_in, float *x_out, int n, int shift) {
  ...
  alpha = (float)(2.0f * M_PI / ((float)n));
  beta = (float)(2.0f * M_PI / ((float)n1));
  gamma = (float)(2.0f * M_PI / ((float)n2));
  for (m1 = 0; m1 < n1; m1++) {
    for (m2 = 0; m2 < n2; m2++) {
      idx = (n1 * m2 + m1) * shift;
      x_out[idx] = 0.0;
...
/*
 ...
 // OUTPUT
 // x_out             - output signal (contains (2 * numRows * numColls)
 elements in order Re(x_in[0][0]), Im(x_in[0][0]), Re(x_in[0][1]),
 Im(x_in[0][1]) and etc.)
 // RESULT
 // Error status
 */
int fft2d(float *x_in, float *x_out, int numRows, int numColls) {
  ...
  for (i = 0; i < numRows; i++) {
    fft(x_in + i * 2 * numColls, x_outTmp + i * 2 * numColls, numColls, 2);
  }
  for (i = 0; i < numColls; i++) {
    fft(x_outTmp + 2 * i, x_out + 2 * i, numRows, 2 * numColls);
  }
  free(x_outTmp);
  return FFT_OK;
}
...
/*
 ...
 // INPUT
 // x_in              - Fourier image of matrix (contains (2 * numRows *
 numColls) elements in order Re(x_in[0][0]), Im(x_in[0][0]), Re(x_in[0][1]),
 Im(x_in[0][1]) and etc.)
 // numRows           - number of rows
 // numColls          - number of columns
 // OUTPUT
...
int fftInverse2d(float *x_in, float *x_out, int numRows, int numColls) {
  ...
  x_outTmp = (float *)malloc(sizeof(float) * (2 * size));
  for (i = 0; i < numRows; i++) {
    fftInverse(x_in + i * 2 * numColls, x_outTmp + i * 2 * numColls, numColls,
               2);
  }
  for (i = 0; i < numColls; i++) {
    fftInverse(x_outTmp + 2 * i, x_out + 2 * i, numRows, 2 * numColls);
  }
  free(x_outTmp);
  return FFT_OK;
}

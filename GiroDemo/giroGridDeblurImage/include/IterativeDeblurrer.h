...
class IterativeDeblurrer : public ImageDeblurrer {
 public:
  ...
  static constexpr int CONVERGENCE_OK = 0;
  static constexpr int NOT_INITIALIZED = 1;
  static constexpr int MAX_ITERATIONS_REACHED = 2;
  static constexpr int ITERATION_FAILED = 3;
  ...
  virtual void operator()(const uint8_t* inputImageData, int inputImagePpln,
                          ...
                          int outputImagePpln) = 0;

  bool isBestIteration() const { return mUseBestIteration; }

  void setUseBestIteration(bool useBestIteration) {
    mUseBestIteration = useBestIteration;
  }

  point_value_t* getBestIterationImage() const { return mBestIterationImage; }
 ...
 protected:
  ...
  virtual void doIterations(
      const GyroBlurKernelBuilder& currentkernelBuilder) = 0;

  virtual void doIterations(const SparseBlurKernel* blurKernels,
                            int blurKernelsCount) = 0;
  ...
  void blur(const point_value_t* inputImageData,
            const SparseBlurKernel* blurKernels, int blurKernelsCount,
            point_value_t* outputImageData, bool transposeKernel = false);
 ...
 private:
  ...
  point_value_t* mBestIterationImage;

  void setBorderImposers();
  void setup(int imageWidth, int imageHeight, void* pExternalMemory);
  void clear();
};

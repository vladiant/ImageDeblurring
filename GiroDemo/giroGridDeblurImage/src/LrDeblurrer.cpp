...
void LrDeblurrer::doIterations(
  ...
  do {
    blur(mCurrentDeblurredImage, currentkernelBuilder, mWeightImage, false);

    blur(mWeightImage, currentkernelBuilder, mReblurredImage, true);
...
void LrDeblurrer::doIterations(const SparseBlurKernel* blurKernels,
  ...
  do {
    blur(mCurrentDeblurredImage, blurKernels, blurKernelsCount, mWeightImage,
         false);

    blur(mWeightImage, blurKernels, blurKernelsCount, mReblurredImage, true);
...
void LrDeblurrer::prepareIterations(
    const GyroBlurKernelBuilder& currentkernelBuilder) {
  size_t imageBlockSize = mImageWidth * mImageHeight * sizeof(point_value_t);
  ...
  // initial approximation
  blur(mCurrentDeblurredImage, currentkernelBuilder, mBlurredImage, true);
  memset(mReblurredImage, 0, imageBlockSize);
}

void LrDeblurrer::prepareIterations(const SparseBlurKernel* blurKernels,
                                    int blurKernelsCount) {
  size_t imageBlockSize = mImageWidth * mImageHeight * sizeof(point_value_t);
  ...
  // initial approximation
  blur(mCurrentDeblurredImage, blurKernels, blurKernelsCount, mBlurredImage,
       true);
  memset(mReblurredImage, 0, imageBlockSize);
}

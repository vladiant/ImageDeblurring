...
int GyroDataReader::readDataFromJPEGFile(const std::string& fileName) {
  char* buf;
...
unsigned int GyroDataReader::calcNumberOfGyroSamplesDuringExposure(
  ...
  Time exposureStartTimeStamp = capture_timestamp - exposureDuration;

  return calcNumberOfGyroSamplesInTimeInterval(exposureStartTimeStamp,
                                               capture_timestamp);
}

unsigned int GyroDataReader::calcNumberOfGyroSamplesInTimeInterval(
    Time startTime, Time endTime) {
  unsigned int numberOfSamplesInTimeInterval = 0;

  if (startTime >= endTime) {
    return numberOfSamplesInTimeInterval;
  }

  for (unsigned int j = 0; j < total_samples; j++) {
    if ((timestamp[j] >= startTime) && (timestamp[j] <= endTime)) {
      numberOfSamplesInTimeInterval++;
    }
  }
...
void GyroDataReader::getAngularVelocitiesDuringExposure(Time exposureDuration,
  ...
  getAngularVelocitiesInInterval(exposureStartTimeStamp, capture_timestamp,
                                 timeStamp, wX, wY, wZ);
}

void GyroDataReader::getAngularVelocitiesInInterval(Time startTime,
                                                    Time endTime,
                                                    Time* timeStamp, float* wX,
                                                    float* wY, float* wZ) {
  int j = 0;

  for (unsigned int i = 0; i < total_samples; i++) {
    if ((timestamp[i] >= startTime) && (timestamp[i] <= endTime)) {
...
void GyroDataReader::getAngularValuesInInterval(
    Time startTime, Time endTime,
    vector<TimeOrientation<Versor<float> > >& samples) {
  samples.clear();

  unsigned int samplesSize = total_samples - SAMPLES_TO_AVERAGE;
  for (unsigned int i = 0; i < samplesSize; i++) {
    TimeOrientation<Versor<float> > CurrentSample =
        pAngularVelocityIntegrator->orientations().orientations()[i];
    Time currentTimeStamp = CurrentSample.time;
    Time nextTimeStamp;

    // Extra start point for the interpolation.
    if (i < samplesSize - 1) {
      nextTimeStamp =
          pAngularVelocityIntegrator->orientations().orientations()[i + 1].time;
      if ((currentTimeStamp < startTime) && (nextTimeStamp > startTime)) {
    ...
    // Extra end point for the interpolation.
    if (i < samplesSize - 1) {
      nextTimeStamp =
          pAngularVelocityIntegrator->orientations().orientations()[i + 1].time;
      if ((currentTimeStamp < endTime) && (nextTimeStamp > endTime)) {
        TimeOrientation<Versor<float> > nextSample =
            pAngularVelocityIntegrator->orientations().orientations()[i + 1];

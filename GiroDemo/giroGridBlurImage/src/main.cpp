...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    std::cout << "\nProgram to blur image using\nthe gyro data from the "
                 "parsegyro image file.\n"
              << std::endl;
    std::cout << "Usage: " << argv[0]
              << "  image_file  parsegyro_image_data_file";
    return 0;
  }
  ...
  cv::Mat initialImage = cv::imread(argv[2]);
  ...
  for (int gridCenterY = gridStepY / 2; gridCenterY < imageHeight;
    ...
    for (int gridCenterX = gridStepX / 2; gridCenterX < imageWidth;
      ...
      // Patch processing.
      cv::Mat patchImageFFT(patchHeight, patchWidth, CV_32FC1);
      patchImage.copyTo(patchImageFFT);
      cv::dft(patchImageFFT, patchImageFFT);

      cv::Mat kernelImageFFT(patchHeight, patchWidth, CV_32FC1);
      ...
      cv::dft(kernelImage, kernelImageFFT);

      cv::mulSpectrums(patchImageFFT, kernelImageFFT, patchImageFFT,
                       cv::DFT_COMPLEX_OUTPUT);

      cv::dft(patchImageFFT, patchImage,
      ...
      cv::imshow("Blurred Image", outputGrayImage);

...
void Sset(cv::Mat &imga, cv::Mat &imgf, float so2 = 1e-6) {
  ...
  for (int row = 0; row < imga.rows; row++) {
    for (int col = 0; col < imga.cols; col++) {
      ...
      ((float *)(imgf.data + row * imgf.step))[col] = s2 / s3;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    /*
    //R matrix updated
    for(int row=0;row<imglr->height;row++)
            for(int col=0;col<imglr->width;col++)
                    {
                            ((float*)(imglr->imageData +
    row*imglr->widthStep))[col]=1.0-((float*)(imgls->imageData +
    row*imgls->widthStep))[col];
                    }
